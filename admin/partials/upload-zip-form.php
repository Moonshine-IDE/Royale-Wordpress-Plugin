...
<div class="wrap" id="rwp-upload-zip-form" class="rwp-upload-zip-form">
  <h1 class="wp-heading-inline"><?= $page_title ?? __( 'Upload a new script', 'royale-wordpress-plugin' ) ?></h1>
  ...
  <form method="POST" action="" enctype="multipart/form-data">
    ...
    <table class="form-table" role="presentation">
        ...
        <tr class="form-field form-required">
		      <th scope="row">
            <label for="shortcode-name"><?= __( 'Shortcode name', 'royale-wordpress-plugin' ) ?> <span class="description">(<?= __( 'required', 'royale-wordpress-plugin' ) ?>)</span></label>
          </th>
		      <td>
        ...
        <tr class="form-field">
          <th scope="row">
            <label for="shortcode-description"><?= __( 'Shortcode description', 'royale-wordpress-plugin' ) ?> </label>
          </th>
          <td>
        ...
        <tr class="form-field form-required">
		      <th scope="row">
            <label for="file">
              <?= __( 'Zip file', 'royale-wordpress-plugin' ) ?>
              <?php if( $is_file_upload_required ) : ?>
                <span class="description">(<?= __( 'required', 'royale-wordpress-plugin' ) ?>)</span>
              <?php endif; ?>
            </label>
          </th>
		      <td>
    ...
    <p class="submit">
      <input type="submit" name="upload_file" id="upload_file" class="button button-primary" value="<?= $submit_button_text ?? __( 'Upload', 'royale-wordpress-plugin' ) ?>">
      <a href="<?= admin_url('admin.php?page=royale_wordpress_plugin_menu') ?>" class="button button-cancel"><?= __( 'Cancel', 'royale-wordpress-plugin' ) ?></a>
    </p>
  </form>
</div>